...
tape('fetch should error for unknown protocol', function (assert) {
  fetch('unprotocol', 'location', 'version', 'directory', log, function (err) {
...
tape('fetch should do nothing for null protocol (current directory)', function (assert) {
  fetch(null, null, null, null, log, function (err) {
...
tape('fetch should fetch from filesystem directory', function (assert) {
  tmp.dir(function (err, directory) {
    if (err) throw err
    fetch('file', path.join(__dirname, 'fixtures', 'diamonds'), null, directory, log, function (err, res) {
      assert.ifError(err, 'no error')
...
tape('should fetch from filesystem tar.gz archive', function (assert) {
  tmp.dir(function (err, directory) {
    if (err) throw err
    fetch('file', path.join(__dirname, 'fixtures', 'diamonds.tar.gz'), null, directory, log, function (err, res) {
      assert.ifError(err, 'no error')
      var files = fs.readdirSync(directory)
      spok(assert, files, [ 'README.md', 'data.csv' ])
      assert.end()
...
tape('should fetch from filesystem zip archive', function (assert) {
  tmp.dir(function (err, directory) {
    if (err) throw err
    fetch('file', path.join(__dirname, 'fixtures', 'hello.zip'), null, directory, log, function (err, res) {
      assert.ifError(err, 'no error')
      var files = fs.readdirSync(directory)
...
tape('should fetch from a Github repo', function (assert) {
  tmp.dir(function (err, directory) {
    if (err) throw err
    fetch('github', 'stencila/examples/diamonds', 'master', directory, log, function (err, res) {
      assert.ifError(err, 'no error')
      var files = fs.readdirSync(directory)
...
tape('should fetch from a Dropbox shared folder', function (assert) {
  tmp.dir(function (err, directory) {
    if (err) throw err
    fetch('dropbox', 'el77xzcpr9uqxb1/AABJIkDNXo_-sKnrUtQvCxC4a', null, directory, log, function (err, res) {
      assert.ifError(err, 'no error')
      var files = fs.readdirSync(directory)
